...
$sql2 = "SELECT DISTINCT
		...
		AND (date(doc_sign.event_time) = '2016-01-29')
		-- 481958 è il primo atto su FS
		-- AND (registrazione.history_id > 481958)

		AND (registrazione.numero_protocollo >= 87)
...
while ($row2 = $rs2->fetchRow(DB_FETCHMODE_ASSOC)) {
	if (DB::isError($row2)) {
		...
		throw new Exception('Errore 1 nella select per la visualizzazione degli albi pretori nel DB');
		die($rs2->getMessage());
	}
	else {
		...
		if (($row2['tipologia'] == "principale") AND ($row2['num_atto'] != $nr_atto)){
			...
			if ($nr_aree_corrispondenti <> 1) {
				echo "<p><strong>Attenzione!</strong> Errore di ambiguita
					...
					<S></S>i e' verificato un errore durante
						l'esecuzione della query <br> \"$sql\".";
				throw new Exception('Errore 2 nella select per la visualizzazione degli albi pretori nel DB');
				die($rs->getMessage());
			}
			if (DB::isError($rs)) {
				echo "<p><strong>Attenzione!</strong>Si e' verificato un errore durante
						l'esecuzione della query <br> \"$sql\".";
				throw new Exception('Errore 3 nella select per la visualizzazione degli albi pretori nel DB');
				die($rs->getMessage());
			}
			while ($row = $rs->fetchRow(DB_FETCHMODE_ASSOC)) {
				if (DB::isError($row)) {
					...
					$result = FALSE;
					throw new Exception('Errore 4 nella select per la visualizzazione degli albi pretori nel DB');
					die($rs->getMessage());
					//die($rs->getMessage());
				}
				else {
