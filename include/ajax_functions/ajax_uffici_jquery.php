...
$i = 0;
$sql = "SELECT id_Servizi_Ente, Ente, Servizio FROM Servizi_Ente WHERE Servizio LIKE '".mysql_real_escape_string($parts[$i])."%' ORDER BY id_Servizi_Ente ASC LIMIT 0, 17;";
...
while ($row = $rs->fetchRow(MDB2_FETCHMODE_ASSOC)) {
	$a_json_row["id"] = $row['id_servizi_ente'];
	$a_json_row["value"] = $row['servizio'];
	$a_json_row["label"] = $row['servizio']." - ".$row['ente'];
